...
%%%=================EXPORTED FUNCTIONS=================
update(Tab, Key, Default, Fun, Lock0, LockTime, TimeOut, Args) when LockTime > TimeOut ->
...
transaction(TabKeys, Fun, Lock0, LockTime, TimeOut, Args) when LockTime > TimeOut ->
	Lock = make_lock(Lock0),
	...
	try
		...
		case catch Fun(Args, add_default(Defaults, size(TabKeys2), TabKeys2, [])) of
			{ok, R} ->
...
%%				Err
		...
		_E1:E2:E3 ->
			...
			?ErrDb([transaction_err, {reason, E2}, {tab_keys, TabKeys}, {lock, Lock},
				{lockTime, LockTime}, {timeout, TimeOut}, E3]),
			{err, E2}
	end.
	...
	case reads_receive(TabKeys1, Ref, Lock, LockTime) of
		...
		{retry_simple, TabKeys2} ->
			if
				...
				true ->
					erlang:error([reads_timeout, [{tab_keys, TabKeys2}, {lock, Lock},
...
transaction_sleep(TabKeys2, Ref, Lock, LockTime, TimeOut, SleepTime, Now) ->
	if
		...
		true ->
			erlang:error([transaction_timeout, [{tab_keys, TabKeys2}, {lock, Lock},
				{lock_time, LockTime}, {timeout, TimeOut}]])
	end.
...
writes_receive(TabKeys, Ref, Lock) ->
	...
	%% Lock锁超时了
		{{_Local, Ref}, {err_lock, OldLock, OldCode, OldFrom, IsMulti}} ->
			erlang:error([transaction_timeout, [{tab_keys, TabKeys}, {lock, Lock},
...
write(Tab, Key, Val, Lock) ->
	Pid = wx_db_server:tab_to_pid(Tab),
	write(Pid, Tab, Key, Val, Lock).
